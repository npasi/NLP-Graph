...
# Allow running this file directly from anywhere (so `import src.*` works)
_PROJECT_ROOT = Path(__file__).resolve().parents[1]  # .../NLP-Graph
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

from src.chapter_splitter import split_into_chapters, split_into_chapters_with_debug
from src.utils.io import data_dir, ensure_dir, safe_slug, write_json, write_text

logger = logging.getLogger(__name__)

_GUTENBERG_FOOTER_MARKERS = [
    r"^\*\*\*\s*END OF (THE )?PROJECT GUTENBERG EBOOK[^\r\n]*\*\*\*\s*$",
    r"^END OF (THE )?PROJECT GUTENBERG EBOOK[^\r\n]*$",
    r"^START:\s*FULL LICENSE\s*$",
    r"^THE FULL PROJECT GUTENBERG.*LICENSE\s*$",
]


def _strip_gutenberg_footer(text: str) -> str:
    for pat in _GUTENBERG_FOOTER_MARKERS:
...
def split_only(
    *,
    ...
    out_root = ensure_dir(data_dir() / "output" / "chapters" / bid)
    if write_debug:
    ...
    for ch in chapters:
        ...
        ch_text = _strip_gutenberg_footer(str(ch["text"]))
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--book-id", default=None, help="Optional output folder name under data/output/chapters/<book-id>/")
