...
def graph_edit_distance(
    ...
    try:
        ...
        if ged is None:
            # nx returns None when timeout elapses without finding the optimum;
            # fall back to the upper-bound via optimize_graph_edit_distance
            ub_gen = nx.optimize_graph_edit_distance(G1, G2)
            ged = next(ub_gen)   # first (possibly loose) upper bound
